...
def build_student_report(student: dict, output_path: str, school_name: str = SCHOOL_NAME,
    ...
    # ── Overall summary ──────────────────────────────────────────────────────
    _module_sections = list(student["modules"].values())
    _journals = student.get("journals", [])
    submitted_count = sum(
        1 for mod in _module_sections
        for _, s in mod if (isinstance(s, float) and s > 0.0) or s == "?"
    ) + sum(1 for _, s in _journals if s)
    total_assignments = sum(len(a) for a in _module_sections) + len(_journals)

    # Use weighted grade from Column D; fall back to computed average if absent
    all_scores = [score for mod in _module_sections
    ...
    if journals:
        ...
        j_col_widths = [
            content_width * 0.70,   # Journal name
            content_width * 0.30,   # Status
        ]
        j_table_rows = [[
            Paragraph("Journal", _STYLE_COL_HDR),
            Paragraph("Status",  _STYLE_COL_HDR_C),
        ]]
        j_row_styles = [
            ("BACKGROUND",    (0, 0), (-1, 0), LIGHT_BG),
            ("LINEBELOW",     (0, 0), (-1, 0), 1, SCHOOL_BLUE),
        ...
        for i, (asgn_name, submitted) in enumerate(journals):
            row_idx = i + 1
            bg = LIGHT_BG_PALE if row_idx % 2 == 0 else WHITE
            if submitted:
                status_para = Paragraph(
                    '<font size="7" color="#1A7340"><b>Submitted</b></font>',
                    _STYLE_ROW_VAL)
            else:
                status_para = Paragraph(
                    '<font size="7">Not Submitted</font>', _STYLE_ROW_VAL)
        ...
        story.append(KeepTogether([journals_banner, j_assign_table]))
        story.append(Spacer(1, 10))
...
def load_grades(xlsx_path: str) -> list[dict]:
    ...
    for row in rows[1:]:
        ...
        for col_idx, asgn_name in assignment_cols:
            ...
            if jnum is not None:
                # Any non-blank cell value counts as submitted
                submitted = raw_cell is not None and str(
                    raw_cell).strip() != ""
                journals.append((asgn_name, submitted))
            else:
                score = raw_cell
